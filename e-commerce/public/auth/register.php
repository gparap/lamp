		...
		<div class="container py-0">
			<div class="row mb-4 mb-lg-5">
				<div class="col-md-8 col-xl-6 text-center mx-auto">
					...
					<h2 class="fw-bold">Welcome guest</h2>
				</div>
			</div>
					...
					<div class="card">
						<div
							...
							<form method="post" data-bs-theme="light">
								...
								<div class="mb-3">
									<button class="btn btn-primary shadow w-100 d-block" type="submit" name="submit">Register</button>
								</div>
							</form>
							<?php
							//Perform the registration process
							if (isset ( $_POST ['submit'] )) {
								//get user registration data into an array
								$registration_data['username'] = $_POST ['username'];
								$registration_data['email'] = $_POST ['email'];
								$registration_data['firstname'] = $_POST ['firstname'];
								$registration_data['lastname'] = $_POST ['lastname'];
								$registration_data['address'] = $_POST ['address'];
								$registration_data['postal_code'] = $_POST ['postal_code'];
								$registration_data['phone'] = $_POST ['phone'];
								$registration_data['password'] = $_POST ['password'];
								$registration_data['password-confirm'] = $_POST ['password-confirm'];

								//TODO: validate input

								//register user
								require_once UTILS_PATH . 'functions.php';
								$is_user_registered = register_user ( $registration_data );

								//redirect user back to login view
								if ($is_user_registered['result'] == true) {
									echo "<script>alert('" . $is_user_registered['info'] . "')</script>";
									echo "<script>window.location.href='" . PUBLIC_URL . "auth/login.php'</script>";
								} else {
									echo "<script>alert('" . $is_user_registered['info'] . "')</script>";
									//TODO: re-fill the fields after failure so user don't have to again
								}
							}
							?>
						</div>
